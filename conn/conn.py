...
def plotMatsEE(): 
    ...
    fig.savefig(dataFolder+'connEE_smat_viridis.png', dpi=1000)
    ...
    fig.savefig(dataFolder+'connEE_pmat_virids.png', dpi=1000)
    ...
    fig.savefig(dataFolder+'connEE_wmat_viridis.png', dpi=1000)
...
def plotMatsEEHoriz(): 
    ...
    fig.set_figheight(8)
    fig.set_figwidth(15)
    fig.savefig(dataFolder+'connEE_smat_hor_viridis.png', dpi=600)
...
def plotMatsEI():  
    ...
    for i,label in enumerate(labels):
        plt.subplot(len(labels),1,i+1)
        xylims = (bins['inh'][0][0], bins['inh'][-1][-1], bins['inh'][0][0], bins['inh'][-1][-1])
        im=plt.imshow(rescale(smat[label], bins['inh'], 0.01), origin='lower', interpolation='None', aspect='auto', extent=xylims, vmin=0, vmax=maxVal)
        #title('E -> '+label)
        if i != 1:
            plt.setp(plt.gca().get_xticklabels(), visible=False)
        # if i == 1:
    ...
    fig.savefig(dataFolder+'connEI_smat_viridis.png', dpi=600)
    ...
    for i,label in enumerate(labels):
        plt.subplot(len(labels),1,i+1)
        xylims = (bins['inh'][0][0], bins['inh'][-1][-1], bins['inh'][0][0], bins['inh'][-1][-1])
        im=plt.imshow(rescale(pmat[label], bins['inh'], 0.01), origin='lower', interpolation='None', aspect='auto', extent=xylims, vmin=0, vmax=maxVal)
        #title('E -> '+label)
        if i != 1:
            plt.setp(plt.gca().get_xticklabels(), visible=False)
        #plt.setp(plt.gca().get_yticklabels(), visible=False)
    ...
    fig.savefig(dataFolder+'connEI_pmat_viridis.png', dpi=600)
    ...
    for i,label in enumerate(labels):
        plt.subplot(len(labels),1,i+1)
        xylims = (bins['inh'][0][0], bins['inh'][-1][-1], bins['inh'][0][0], bins['inh'][-1][-1])
        im=plt.imshow(rescale(wmat[label], bins['inh'], 0.01), origin='lower', interpolation='None', aspect='auto', extent=xylims, vmin=0, vmax=maxVal)
        #title('E -> '+label)
        # if i == 1:
        #      xlabel(' . ', fontsize=18); 
        if i != 1:
    ...
    fig.savefig(dataFolder+'connEI_wmat_viridis.png', dpi=1000)
    #show()
...
# Format:
# Key tuple format for E->E represented (dataset, post pop, post layer) (e.g. ('W+AS_norm', 'IT', 'L6'))
## Each dict entry is a 3x3 array corresponding to pre and post-syn spatial regions (bins) as [post, pre]
...
# 11a. E->I connections (based on Apicella, 2011)
## 11a.i) Use final matrices for IT cells but group in L1+L2/3+4, L5A/B and L6 for FS and LTS following Apicella
## To downsample W+AS matrices, group bins into these 3 layers, and use max value so overall E->I wil be stronger than E->E
## 11a.ii) Use same for VIP and NGF cells; not much data but can conclude they have local E inputs (Tremblay, 2016: Sohn, 2016; Budinger, 2018; Garcia, 2015)

## Create empty dicts to store data
## Note format here differs from above: key tuple indicates pre, post (e.g. ('E','SOM'))
## Previous key tuple format for E->E indicated: (dataset, post pop, post layer) (e.g. ('W+AS_norm', 'IT', 'L6'))
## Each dict entry is a 3x3 array corresponding to pre and post-syn spatial regions (L1,2/3+4, L5A+5B and L6) [post, pre]

inhTypes = ['SOM', 'PV', 'VIP', 'NGF']  # inhibitory types
for post in inhTypes:
    pmat[('E', post)] = np.ones((3,3))
    wmat[('E', post)] = np.ones((3,3))
...
for celltype in inhTypes:
    ## postsyn L2/3,L4
    pmat[('E', celltype)][0,0] = pmat[('W+AS_norm', 'IT', 'L2/3,4')][:,W_234_bins].mean()*EtoIprobFactor
    pmat[('E', celltype)][0,1] = pmat[('W+AS_norm', 'IT', 'L2/3,4')][:,W_5_bins].mean()*EtoIprobFactor
    pmat[('E', celltype)][0,2] = pmat[('W+AS_norm', 'IT', 'L2/3,4')][:,W_6_bins].mean()*EtoIprobFactor
    wmat[('E', celltype)][0,0] = wmat[('W+AS_norm', 'IT', 'L2/3,4')][:,W_234_bins].mean()
    wmat[('E', celltype)][0,1] = wmat[('W+AS_norm', 'IT', 'L2/3,4')][:,W_5_bins].mean()
    wmat[('E', celltype)][0,2] = wmat[('W+AS_norm', 'IT', 'L2/3,4')][:,W_6_bins].mean()

    ## postsyn 5A,5B
    pmat[('E', celltype)][1,0] = pmat[('W+AS_norm', 'IT', 'L5A,5B')][:,AS_234_bins].mean()*EtoIprobFactor
    pmat[('E', celltype)][1,1] = pmat[('W+AS_norm', 'IT', 'L5A,5B')][:,AS_5_bins].mean()*EtoIprobFactor
    pmat[('E', celltype)][1,2] = pmat[('W+AS_norm', 'IT', 'L5A,5B')][:,AS_6_bins].mean()*EtoIprobFactor
    wmat[('E', celltype)][1,0] = wmat[('W+AS_norm', 'IT', 'L5A,5B')][:,AS_234_bins].mean()
    wmat[('E', celltype)][1,1] = wmat[('W+AS_norm', 'IT', 'L5A,5B')][:,AS_5_bins].mean()
    wmat[('E', celltype)][1,2] = wmat[('W+AS_norm', 'IT', 'L5A,5B')][:,AS_6_bins].mean()

    ## postsyn L6
    pmat[('E', celltype)][2,0] = pmat[('W+AS_norm', 'IT', 'L6')][:,W_234_bins].mean()*EtoIprobFactor
    pmat[('E', celltype)][2,1] = pmat[('W+AS_norm', 'IT', 'L6')][:,W_5_bins].mean()*EtoIprobFactor
    pmat[('E', celltype)][2,2] = pmat[('W+AS_norm', 'IT', 'L6')][:,W_6_bins].mean()*EtoIprobFactor * 1.5 # increase L6 CT/IT->I (Yamawaki & Shepeherd 2015)
    wmat[('E', celltype)][2,0] = wmat[('W+AS_norm', 'IT', 'L6')][:,W_234_bins].mean()
    wmat[('E', celltype)][2,1] = wmat[('W+AS_norm', 'IT', 'L6')][:,W_5_bins].mean()
    wmat[('E', celltype)][2,2] = wmat[('W+AS_norm', 'IT', 'L6')][:,W_6_bins].mean()
...
pmat[('E','SOM')][1,0] = 1.5*pmat[('E','SOM')].max() #pmat[('W+AS_norm', 'IT', 'L5A,5B')][:,2:4].max() # increase IT L2/3 -> LTS L5A/B prob
pmat[('E','PV')][1,0] = 0.5*pmat[('E','PV')].min() #pmat[('W+AS_norm', 'IT', 'L5A,5B')][:,2:4].min() # decrease IT L2/3 -> FS L5A/B prob
#wmat[('E','PV')][1,0] = wmat[('W+AS_norm', 'IT', 'L5A,5B')][:,2:4].min() # decrease IT L2/3 -> FS L5A/B weight

## 11c. IT/PT L5A/B  -> FS L5A/B = STRONG (compared to IT/PT L5A/B -> LTS L5A/B)  
pmat[('E','PV')][1,1] = 1.5*pmat[('E','PV')].max() #pmat[('W+AS_norm', 'IT', 'L5A,5B')][:,2:11].max() # increase IT/PT L5A/B -> FS L5A/B prob (use L2/3 values)
pmat[('E','SOM')][1,1] = 0.5*pmat[('E','SOM')].min() #pmat[('W+AS_norm', 'IT', 'L5A,5B')][:,5:6].min() # decrease IT/PT L5A/B -> LTS L5A/B prob (>0)
#pmat['SOM'][1,1] = pmat[('W+AS_norm', 'IT', 'L5A,5B')][:,5:11].min()

## 11d. Decrease E->I probs for VIP, NGF (unclear evidence: Tremblay, 2016: Sohn, 2016; Budinger, 2018; Garcia, 2015))
EtoIprobFactorVIP = 1.0
EtoIprobFactorNGF = 1.0
pmat[('E','VIP')] *= EtoIprobFactorVIP
pmat[('E','NGF')] *= EtoIprobFactorNGF

## 11.e Calculate smat = pmat * wmat
for itype in inhTypes:
    smat[('E',itype)] = pmat[('E',itype)] * wmat[('E',itype)]


# 12. I->E/I connections (Tremblay, 2016: Sohn, 2016; Budinger, 2018; Naka et al 2016; Garcia, 2015)
