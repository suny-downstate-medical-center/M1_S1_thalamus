...
def plot4Dparams(dataFolder, batchLabel, df, par1, par2, par3, par4, val, valLabel=None, groupStat='mean', cluster=False, normRange=False, saveFile=None):
    ...
    if saveFile:
        ...
        plt.savefig(dataFolder+batchLabel+'/heatmap_'+par1+'_'+par2+'_'+par3+'_'+par4+'_'+val+normStr+'.png')
    plt.show()
    return dfgroup3
...
def popRateAnalysis(dataFolder, batchLabel, loadAll, pars=['IEweights_0','IIweights_0','IEweights_2','IIweights_2'], 
    groupPars = [], vals=['IT2','IT4','IT5A', 'IT5B', 'PT5B','IT6','CT6'], query=None, plotLine=False):
    # load data 
    ...
    if len(pars) == 1:
        ...
        dfpiv=plot4Dparams(dataFolder, batchLabel, dfpop, par1=pars[0], par2=pars[1], par3=pars[2], par4=pars[3], val=vals, valLabel='rate (Hz)',  groupStat='first', normRange=1)
