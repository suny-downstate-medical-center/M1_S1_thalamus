...
if cfg.ihGbarZD is not None and 'PT5B_full2' not in loadCellParams:
    ...
    # cellRule = netParams.importCellParams(label='PT5B_full2', conds={'cellType': 'PT2', 'cellModel': 'HH_full'},
    #   fileName='cells/PTcell.hoc', cellName='PTcell', cellArgs=[0,0,0], somaAtOrigin=True)
    # nonSpiny = ['apic_0', 'apic_1']
    # netParams.addCellParamsSecList(label='PT5B_full2', secListName='perisom', somaDist=[0, 50])  # sections within 50 um of soma
    # for sec in nonSpiny: cellRule['secLists']['perisom'].remove(sec)
    # cellRule['secLists']['alldend'] = [sec for sec in cellRule.secs if ('dend' in sec or 'apic' in sec)] # basal+apical
    ...
    #   cellRule['secs'][secName]['weightNorm'] = [2.0/3.0] * cellRule['secs'][secName]['geom']['nseg']

    # Reduce dend Na to avoid dend spikes (compensate properties by modifying axon)
    # for secName in cellRule['secLists']['alldend']:
    #   cellRule['secs'][secName]['mechs']['nax']['gbar'] = 0.0153130368342 * 0.25 
    # cellRule['secs']['axon']['mechs']['nax']['gbar'] = 0.0153130368342  * 11 
    # cellRule['secs']['axon']['geom']['Ra'] = 137.494564931 * 0.005
    # netParams.addCellParamsWeightNorm('PT5B_full2', 'conn/PT5B_full_weightNorm.pkl', threshold=cfg.weightNormThreshold)  # load weight norm
    # netParams.saveCellParamsRule(label='PT5B_full2', fileName='cells/PT5B_full2_cellParams.pkl')
...
## Local populations
netParams.popParams['PT5B_1'] = {'cellModel': cfg.cellmod['PT5B'], 'cellType': 'PT', 'ynormRange': layer['5B'], 'density': 0.5*density[('M1','E')][3]}#, 'xRange': [100,100], 'yRange': [735,735]}
...
if cfg.addNetStim:
    ...
    for key in [k for k in dir(cfg) if k.startswith('GroupNetStim')]:
        params = getattr(cfg, key, None)
        nstype, numStims, pop, ynorm, cellRule, secList, allSegs, synMech, start, interval, noise, number, weight, delay = \
        [params[s] for s in 'nstype', 'numStims', 'pop', 'ynorm', 'cellRule', 'secList', 'allSegs', 'synMech', 'start', 'interval', 'noise', 'number', 'weight', 'delay'] 
...
if cfg.netClamp:
    ...
    if cfg.netClampCreateAllConns:
        ...
        #    cell['spkTimes'] = cell['spkTimes'][:2:] # list(random.uniform(0,cfg.duration,(rate*cfg.duration/1000)))
